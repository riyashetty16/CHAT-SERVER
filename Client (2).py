...
def serverListen(serverSocket):
	while True:
		...
		if msg == "/viewRequests":
			...
			copy_file(filename,destination_file)

		else:
			print(msg)

#Files being copied into Received.txt file for backups/copy of and along with original file		
def copy_file(filename, destination_file):
                    try:
                        shutil.copy(filename, destination_file)
                        print(f"File '{filename}' copied to '{destination_file}' successfully.")
                    except FileNotFoundError:
...
def main():
	...
	state["username"] = input("Welcome to the Chatroom! Please enter your username: ")
	state["groupname"] = input("Please enter the name of the group: ")
	state["alive"] = False
	state["joinDisconnect"] = False
	state["inputMessage"] = True
	client_socket.send(bytes(state["username"],"utf-8"))
	client_socket.recv(1024)
	client_socket.send(bytes(state["groupname"],"utf-8"))
	...
	waitUserInputThread = threading.Thread(target=waitUserInput,args=(client_socket,))
	waitServerListenThread = threading.Thread(target=waitServerListen,args=(client_socket,))
	userInputThread = threading.Thread(target=userInput,args=(client_socket,))
	serverListenThread = threading.Thread(target=serverListen,args=(client_socket,))
